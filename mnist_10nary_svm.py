...
size_list = np.arange(1000,10001,2000)
C_list = np.arange(1,1.1,0.2)
...
for i,size in enumerate(size_list):
    ...
    for j,c in enumerate(C_list):
        accu_c_lst[j] = accu_varying_samples(size,train_img,train_lbl,validation_img,validation_lbl,c=c,repeat = 1,kernel = kernel)
    C_opt = C_list[np.argmax(accu_c_lst)]
    accu_data_lst[i] = accu_c_lst.max()
    # use test image to test the accuracy
    accu_test_data[i] = accu_varying_samples(size,train_img,train_lbl,test_img,test_lbl,c=C_opt,repeat = 1,kernel = kernel)
    print(size, C_opt, accu_data_lst[i], accu_test_data[i])
    
outfile = 'MNIST_result/MNIST_10nary_SVM_'+str(kernel)+'_data_Accu'
    ...
    for j, M in enumerate(M_list):
        ...
        for k, c in enumerate(C_list):
            ...
            accu_c_lst[k] = accu_varying_samples(size, pm1_train_img[:,:M], train_lbl,\
                                         pm1_validation_img[:,:M], validation_lbl,c=c, repeat=1,kernel = kernel)
        C_opt = C_list[np.argmax(accu_c_lst)]
        accu_pm1_lst[i,j] = accu_c_lst.max()
        accu_pm1_test[i,j] = accu_varying_samples(size, pm1_train_img[:,:M], train_lbl,\
                                        pm1_test_img[:,:M],test_lbl,c=C_opt,repeat=1,kernel = kernel)
        print(size, M, C_opt, accu_pm1_lst[i,j], accu_pm1_test[i,j])
outfile = 'MNIST_result/MNIST_10nary_SVM_'+str(kernel)+'_pm1_Accu'
